...
class AuthMethodsFromParamsHelper
{
    ...
    private function getAuthMethods()
    {
        ...
        if (Yii::$app->params['useHttpBasicAuth']) {
            $authMethodsArray[] = HttpBasicAuth::class;
        }
        if (Yii::$app->params['useHttpBearerAuth']) {
            $authMethodsArray[] = HttpBearerAuth::class;
        }
        if (Yii::$app->params['useQueryParamAuth']) {
            $authMethodsArray[] = QueryParamAuth::class;
        }
        if ($authMethodsArray == null) {
            throw new NotSupportedException('You must choose at least one auth methods, configure your app\config\params.php for more options.');
        }
