    ...
    foreach ($tripList as $key => $tripItem) {
        ...
        <tr data-key="<?= $tripItem->id ?>" role="row">
            ...
            <td class="text-center text-bold text-danger">
                <?= MyStringHelper::convertIntegerToPrice((! empty($tripItem->money_debt_agency) ? $tripItem->money_debt_agency : ($tripItem->price_customer - $tripItem->bid->price > $tripItem->price_customer / 100 * $agency->percent && ! empty($agency->percent) ? $tripItem->price_customer / 100 * $agency->percent : ($tripItem->price_customer - $tripItem->bid->price > $agency->price ? $agency->price : 0)))) ?>đ
            </td>
