    ...
    <div class="box box-green" style="margin-bottom: 20px; border-top: 0;">
        ...
        <div class="box-body">
            <div class="trip-search">
                ...
                <?php
                $listDriver = Driver::find()->where(['register' => 1])->all();
                $data = ['0' => 'Tất cả'];
                ...
                foreach ($driverList as $driverItem) {
                    ...
                    <tr data-key="<?= $driverItem->id ?>" role="row">
                        ...
                        <td class="d-flex" style="justify-content: space-evenly;">
                            <?php $url_accept = Url::to(['driver/accept', 'id' => $driverItem->id]); ?>
                            <?php $url_reject = Url::to(['driver/delete', 'id' => $driverItem->id]); ?>
                            <?=
                            Html::a('<span class="fa fa-check"></span>', $url_accept, [
                                'title' => 'Xác nhận tài khoản',
                                'data-confirm' => Yii::t('yii', 'Duyệt tài khoản này?'),
                                'data-method' => 'post',
                                'class' => 'btn-success btn ',
                                'style' => 'margin-right:6px;',
                            ]); ?>
                            <a href="/driver/update?id=<?= $driverItem['id'] ?>"
                            class="btn-primary btn" style="margin-right:6px;" title="View" aria-label="View" data-pjax="0">
                            <span class="fa fa-eye" aria-hidden="true"></span>
                            </a>
                            <?=
                            Html::a('<span class="fa fa-trash"></span>', $url_reject, [
                                'title' => 'Xóa tài khoản',
                                'data-confirm' => Yii::t('yii', 'Xóa tài khoản này?'),
                                'data-method' => 'post',
