  ...
  jQuery(document).ready(function ($) {
    ...
    function load_data_search(page = 0){
      ...
      typingTimer = setTimeout(function() {
        ...
        formData.push({ name: 'this_table', value: 'table-driver-debt-collection' });
        ...
        formData.push({ name: 'SearchTripDriver[driver_debt]', value: 'driver_debt_collection' });
        ...
        let url = '/trip-driver/driver-debt-collection?' + url_data;
    ...
    $(document).on('click', '.js-btn-collection-money', function() {
      let id = $(this).data('id');
      if (confirm('Đã thu nợ tài xế?')) {
        $.ajax({
          type: "post",
          url: "/trip-driver/update-driver-debt-collection",
          data: {
            id: id,
          },
          success: function () {
            toastr.success('Đã thu nợ thành công!');
          },
          error: function (response) {
