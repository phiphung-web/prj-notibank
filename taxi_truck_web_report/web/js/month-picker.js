    ...
    for (y = 0; y < 2; y++) {
        ...
        for (m = 0; m < 12; m++) {
            ...
            if ((m + 1) < 10)
                ...
                monthval = "" + (m + 1);
            content += '<span class="col-xs-3 mpr-month month_' + monthval + '">' + MONTHS[m] + '</span>';
        }
        content += '</div></div></div></div>';
    }
    content += '<div class="col-xs-1"> <h5 class="mpr-quickset">Quick Set</h5>';
    content += '<a class="btn btn-info mpr-ytd">This year</a>';
    content += '<a class="btn btn-info mpr-prev-year">Previous year</a>';
    content += '</div>';
    content += '</div>';
    $(document).on('click', '.mpr-month', function (e) {
        e.stopPropagation();
        $month = $(this);
        var __class = $month.attr("class");
        var monthnum = __class.split("_")[1].substring(0, 2);
